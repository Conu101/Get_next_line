...
/*   Updated: 2022/01/04 20:06:34 by ctrouve          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Return values:
	if there is a new line at index len before the end of the file
		malloc the string *line the value of the substring in
		*stat between index 0 and len
		return 1 
	if the file ends before a new line was found
		malloc the string *line with the value of entire *stat
		free(*stat), set it to NULL and return 0 (end of reading)
*/
static int	new_line(char **stat, char **line)
{
	size_t	len;
	char	*temp;
...
Return values:
	if error: -1
	if last reading read 0 bytes and the static is empty: 0 (empty file)
